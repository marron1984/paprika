...
export default async function Leads({
  ...
  if (sp.status) filters.push(`status=eq.${encodeURIComponent(sp.status)}`);
  if (sp.q)
    filters.push(
      `or=(consultant_name.ilike.*${encodeURIComponent(sp.q)}*,consultant_phone.ilike.*${encodeURIComponent(sp.q)}*,resident_name.ilike.*${encodeURIComponent(sp.q)}*)`,
    );
  ...
  return (
    <AdminShell>
          ...
          <tbody>
            {leads.map((l) => (
              <tr key={l.id}>
                ...
                <td>{l.consultant_phone}</td>
                <td>{l.resident_name}</td>
                <td>{l.care_level}</td>
                <td>{l.desired_area}</td>
                <td>{l.source || l.utm_source}</td>
                <td>
      ...
      <section className="mt-6 card p-6">
        <h2 className="text-xl font-black">カンバン表示</h2>
          ...
          {Object.entries(
            ...
            <div className="rounded-2xl bg-slate-50 p-4" key={status}>
              ...
              {items.slice(0, 5).map((l) => (
                <a
                  key={l.id}
                  className="mt-3 block rounded-xl bg-white p-3 text-sm font-bold text-blue-700"
                  href={`/admin/leads/${l.id}`}
                >
                  {l.consultant_name}
                </a>
