...
export async function supabaseFetch<T>(path: string, init: RequestInit = {}, admin = false): Promise<T> {
  const base = assertConfig(url, "NEXT_PUBLIC_SUPABASE_URL");
  const key = assertConfig(admin ? serviceKey : anonKey, admin ? "SUPABASE_SERVICE_ROLE_KEY" : "NEXT_PUBLIC_SUPABASE_ANON_KEY");
...
export async function signInWithPassword(email: string, password: string) {
  ...
  return res.json() as Promise<{ access_token: string; refresh_token: string; user: { id: string; email?: string } }>;
}
